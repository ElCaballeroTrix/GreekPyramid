...
export (String) var rutaCamara
onready var camara=get_tree().get_root().get_node(rutaCamara)
func _ready():
	pass
...
#	get_parent().get_node("Camera2D").offset=Vector2(rand_range(-vector.x,vector.x),rand_range(-vector.y,vector.y))
	camara.offset=Vector2(rand_range(-vector.x,vector.x),rand_range(-vector.y,vector.y))
